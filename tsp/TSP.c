...
#pragma omp parallel for private(i, j) shared(matrizDistancias, coordenadasX, coordenadasY, nCiudades)
	for (i = 0; i < nCiudades; i++)
		...
		{
			double x = pow((coordenadasX[i] - coordenadasX[j]), 2);
			double y = pow((coordenadasY[i] - coordenadasY[j]), 2);
			double distancia = sqrt(x + y);
			// double distancia = sqrt((x+y), 2);
...
void leerFicheroCamino(char filename[], int camino[])
{
	FILE *input_file = fopen(filename, "r");
	if (!input_file)
	{
